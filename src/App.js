...
function App() {
  ...
  const [processedCurrentWeatherData, setProcessedCurrentWeatherData] =
    useState(null);
  ...
  function useRetrieveCurrentWeatherData() {
    useEffect(() => {
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=b1ab53cd687ca486f76e739d8fc1a32b&units=${units}`
      )
        .then((response) => response.json())
        .then((data) => {
